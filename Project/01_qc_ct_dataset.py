#!/usr/bin/env python3
"""
Step 01: Quality control for HECKTOR CT/PET/mask data.

This script reads the extracted HECKTOR 2025 dataset and creates:
    ...
    7. summary_raw.json
    8. summary_normalized.json
    9. center_wise_ct_case_distribution.png
    10. ct_mean_intensity_by_center.png
    11. abnormal_case_count_by_center.png
...
def has_valid_extension(path: Path) -> bool:
    return any(path.name.lower().endswith(ext) for ext in VALID_EXTENSIONS)
...
def json_safe(value: Any) -> Any:
    """
    Convert NumPy and tuple objects into JSON-safe objects.
    """
...
def find_case_files(patient_folder: Path) -> dict[str, Path | None]:
    """
    Locate CT, PET, mask, and optional RTDOSE files.
    """
    ...
    files = [
        p for p in patient_folder.iterdir()
        if p.is_file() and has_valid_extension(p)
    ]
...
def clip_ct(arr: np.ndarray, hu_min: float, hu_max: float) -> np.ndarray:
...
def normalize_ct(arr: np.ndarray, hu_min: float, hu_max: float) -> np.ndarray:
    """
    HU clip followed by z-score normalization.
    """
    arr = clip_ct(arr, hu_min=hu_min, hu_max=hu_max)
...
def compare_geometry(ct_img: sitk.Image, mask_img: sitk.Image) -> dict[str, Any]:
    ...
    return {
        ...
        "ct_size_xyz": tuple(int(x) for x in ct_size),
        "mask_size_xyz": tuple(int(x) for x in mask_size),
...
def summarize_mask(mask_arr: np.ndarray) -> dict[str, Any]:
    """
    Summarize mask labels.

    Label 0 = background
    Label 1 = GTVp / primary tumor
    Label 2 = GTVn / nodal tumor
    """
...
def detect_abnormal_case(
    ...
    pet_read_error: str,
    mask_read_error: str,
    raw_unclipped_stats: dict[str, float | None],
    ...
    raw_min = raw_unclipped_stats.get("min")
    raw_max = raw_unclipped_stats.get("max")
    raw_std = raw_unclipped_stats.get("std")

    if raw_min is None or raw_max is None:
        reasons.append("ct_no_finite_values")
    else:
        if raw_min < abnormal_low:
            reasons.append("low_intensity_outlier")
        if raw_max > abnormal_high:
            reasons.append("high_intensity_outlier")

    if raw_std is not None and raw_std < 1e-6:
...
def process_case(
    ...
    raw_unclipped_stats = compute_stats(np.array([], dtype=np.float32))
    raw_clipped_stats = compute_stats(np.array([], dtype=np.float32))
    norm_stats = compute_stats(np.array([], dtype=np.float32))
    ...
    if has_ct:
        try:
            ...
            raw_unclipped_stats = compute_stats(ct_arr)

            ct_clipped = clip_ct(ct_arr, hu_min=hu_min, hu_max=hu_max)
            raw_clipped_stats = compute_stats(ct_clipped)

            ct_norm = normalize_ct(ct_arr, hu_min=hu_min, hu_max=hu_max)
            norm_stats = compute_stats(ct_norm)
    ...
    if has_mask:
        try:
            ...
            mask_info = summarize_mask(mask_arr)
        except Exception as exc:
    ...
    is_abnormal, abnormal_reason = detect_abnormal_case(
        ...
        raw_unclipped_stats=raw_unclipped_stats,
        mask_info=mask_info,
    ...
    base_record = {
        ...
        "raw_unclipped_min": raw_unclipped_stats["min"],
        "raw_unclipped_max": raw_unclipped_stats["max"],
        "raw_unclipped_mean": raw_unclipped_stats["mean"],
        "raw_unclipped_std": raw_unclipped_stats["std"],
    ...
    if usable_for_ct_training:
        raw_record = {
            **base_record,
            "min": raw_clipped_stats["min"],
            "max": raw_clipped_stats["max"],
            "mean": raw_clipped_stats["mean"],
            "std": raw_clipped_stats["std"],
            "median": raw_clipped_stats["median"],
            "p01": raw_clipped_stats["p01"],
            "p99": raw_clipped_stats["p99"],
        }
...
def process_dataset(
    ...
    return inventory_df, raw_df, norm_df, abnormal_df, dataset_root
...
def make_abnormal_by_center(inventory_df: pd.DataFrame) -> pd.DataFrame:
    result = (
        inventory_df.groupby("center")
        .agg(
...
def save_center_barplot(center_counts: pd.DataFrame, output_png: Path) -> None:
    ...
    ax.set_title(
        "Center-wise CT Case Distribution",
        fontsize=22,
        fontweight="bold",
        pad=18,
    )
...
def save_intensity_boxplot(raw_df: pd.DataFrame, output_png: Path) -> None:
    ...
    ax.set_title(
        "Center-wise CT Mean Intensity Distribution",
        fontsize=14,
        fontweight="bold",
    )
    ax.set_xlabel("Center")
    ax.set_ylabel("Mean CT Intensity After HU Clipping")
...
def save_abnormal_barplot(abnormal_by_center: pd.DataFrame, output_png: Path) -> None:
    ...
    ax.set_xlabel("Center")
    ax.set_ylabel("Number of Flagged Cases")
    ax.grid(axis="y", linestyle="--", alpha=0.35)
    ax.set_axisbelow(True)
...
def print_summary(
    ...
    print(f"Flagged abnormal cases: {total_abnormal}")
    ...
    print(f"Raw CT CSV: {out_dir / 'ct_dataset_raw.csv'}")
    print(f"Normalized CT CSV: {out_dir / 'ct_dataset_normalized.csv'}")
    print(f"Abnormal cases CSV: {out_dir / 'abnormal_cases.csv'}")
    print(f"Center counts CSV: {out_dir / 'center_case_counts.csv'}")
    print(f"Center distribution plot: {out_dir / 'center_wise_ct_case_distribution.png'}")
    print(f"CT intensity plot: {out_dir / 'ct_mean_intensity_by_center.png'}")
    print(f"Abnormal case plot: {out_dir / 'abnormal_case_count_by_center.png'}")
...
def main() -> None:
    ...
    inventory_df, raw_df, norm_df, abnormal_df, dataset_root = process_dataset(
        root=args.root,
        hu_min=args.hu_min,
        hu_max=args.hu_max,
    ...
    abnormal_by_center = make_abnormal_by_center(inventory_df)
